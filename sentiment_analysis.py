...
def scrp(n1,n2):
    import scrape3 
    s1,ps=scrape3.search(n1)
    reviews=scrape3.appReviews(ps)
    pos1=0
    pos2=0
    neg1=0
    neg2=0
    ...
    for i in range(0,40):
        ...
        sentence_list = tokenize.sent_tokenize(paragraph.decode('utf-8'))
        for sentence in sentence_list:
            analyzer = SentimentIntensityAnalyzer()
            vs = analyzer.polarity_scores(sentence)
        paragraphSentiments += vs["compound"]
        if((paragraphSentiments/len(sentence_list))>0.0):
            pos1=pos1+1
        if((paragraphSentiments/len(sentence_list))==0.0):
            neu1=neu1+1
        else:
            neg1=neg1+1
        app1=(paragraphSentiments/len(sentence_list))+app1
        print(paragraphSentiments)
        
    s2,ps=scrape3.search(n2)
    reviews=scrape3.appReviews(ps)
    app2=0.0
    for i in range(0,40):
        paragraph=reviews[i]
        ...
        if((paragraphSentiments/len(sentence_list))==0.0):
            ...
            neg2=neg2+1
        app2=(paragraphSentiments/len(sentence_list))+app2
    return(app1,app2,neu1,pos1,neg1,neu2,pos2,neg2)


    
if __name__ == '__main__':
    from scrape3 import *
    n1=raw_input("enter name of app")    
    n2=raw_input("enter name of second app")
    scrp(n1,n2)
